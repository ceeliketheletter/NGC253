...
//double NE_finite_time(double loggamma)
{
    ...
    TF1 qe_mod_diff("qe_mod_diff",QE_mod,(log_prot_gmin-0.5),(log_prot_gmax+0.5),1);
    qe_mod_diff.SetParameter(0,loggamma[0]);
...
// int main(int argc, char **argv){
    ...
    // double magfieldstrength = atof(argv[1]), plindex = atof(argv[2]), Ebreak = atof(argv[3]);
    ...
    /*sprintf(rootfilename,"/Users/CEE/Desktop/NGC_253/test.root");
    sprintf(funcname,"ne_prot_pl_exp_index_%.1f_Ecut_%.1f_log_prot_gmax_%.1f_eff_%.1f",prot_p,log10(prot_high_cutoff),log_prot_gmax,sneff_proton);
    */
    ...
    TF1 qe_norm_diff("qe_kin_diff",QE_kin_diff,log_prot_gmin-.5,6.5,1);
    qe_norm_diff.SetParameter(0,specflag);
    ...
    /*TF1 protdist(funcname,NE_finite_time,log_prot_gmin+logbinsize,log_prot_gmax,1);
    protdist.SetParameter(0,normalization);*/
