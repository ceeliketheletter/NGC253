...
//int main(int argc, char **argv){     
  ...
  std::string prefix = "finding2_a_"; string suffix = ".txt"; 
  int i, j, k;
...
//  double inputlogEcut;
  ...
  for (i=0, inputplleindex=2.01; inputplleindex<=2.7; inputplleindex+=0.138, ++i)
  {  
...
//        myfile << "Given : " << "[" << argv[1] << "  " << argv[2] << "  " << argv[3] << "  " << argv[4] << "  " << argv[5] << "  " << argv[6] << "  " << argv[7] << "]" <<endl;
        myfile << '#' << '\n';
        myfile << "# " << "loggamma" << "\t" << "log(ne_ss)" << endl;
        ...
        for (loggamma=.1; loggamma<=3; loggamma+=logbinsize)
            {
                ...
                myfile << loggamma << "\t" << "\t" << log10(ne_ss) << endl; //to save to a file, use myfile instead of cout   
