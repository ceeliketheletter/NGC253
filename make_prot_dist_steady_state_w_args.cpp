...
double NE_no_escape(double loggamma, double bfield)
{
...
int main(int argc, char **argv){
    ...
    double galBfield = atof(argv[1]), specpar = atof(argv[2]), inputplleindex = atof(argv[3]), inputbplhiindex = atof(argv[4]), inputbpln = atof(argv[5]), inputbplEbreak = atof(argv[6]), inputlogEcut = atof(argv[7]);
    ...
    ofstream myfile;
    myfile.open ("Round1_better.txt");

    myfile << "Output file of: 'make_prot_dist_steady_state_w_args.cpp' " << "\t" << "Round 1" << endl;
    ...
    for (loggamma=.1; loggamma<=11; loggamma+=logbinsize)
    {
        ...
        myfile << loggamma << "\t" << "\t" << ne_ss << endl; //to save to a file, use myfile instead of cout             
    }
    myfile.close();
