...
double wind_speed = 1000; // km/s -- NGC 253
...
const double prot_E_max = 1.e20; // eV
...
{
    /*par[0] = specpar;
     par[1] = inputplleindex;
     par[2] = inputbplhiindex;
     par[3] = inputbpln;
     par[4] = inputbplEbreak;
     par[5] = inputlogEcut;*/
...
/*double NE_no_escape(double loggamma)
{
...
double diff_timescale(double energy, double emispar) // [energy] = eV
{
    ...
    switch (emisflag) {
        case 1: // Lacki et al. 2012 hadronic
            tdiff = 1.0e6*year/sqrt(energy*1./(3.e9));
            break;
        case 2: // Lacki et al. 2012 leptonic
            tdiff = 1.0e7*year/sqrt(energy*1./(3.e9));
            break;
    }
...
double conv_timescale(double energy, double emispar)
{
    double tau_diff = diff_timescale(energy,emispar);
...
{
    //double magfield = par[0];
    double emispar = par[0];
    double gamma = pow(10.0,loggamma[0]);
    double primenergy = gamma*protonmass;
    double factor = primenergy*log(10.0);
    double t_diff = diff_timescale(primenergy,emispar);
    double t_conv = conv_timescale(primenergy,emispar);
    ...
    switch (emisflag) {
        case 1: // Lacki et al. 2012 hadronic
            //intfactor = factor*1./(t_diff*bE_total(loggamma[0],magfield));
            intfactor = factor*1./(t_diff*bE_total(loggamma[0],regBfield));
            break;
        case 2: // Lacki et al. 2012 leptonic
            //intfactor = factor*1./(t_conv*bE_total(loggamma[0],magfield));
            intfactor = factor*1./(t_conv*bE_total(loggamma[0],regBfield));
            break;
    }
    ...
    //return factor*1./(t_diff*bE_total(loggamma[0],magfield));
...
double QE_mod(double *loggamma, double *par) // par[0] = log10(secprotonenergy)
{
    ...
    /*qe_par[0] = specpar;
     qe_par[1] = inputplleindex;
     qe_par[2] = inputbplhiindex;
     qe_par[3] = inputbpln;
     qe_par[4] = inputbplEbreak;
     qe_par[5] = inputlogEcut;*/
    ...
    integ_fac.SetParameter(0,emispar);
    //integ_fac.SetParameters(magfield,emispar);
...
double NE_finite_time(double *loggamma, double *pars)
{
    double norm = pars[0];
    //double magfield = pars[1];
    double gamma = pow(10.0,loggamma[0]);
    ...
    /*funcpars[1] = specpar;
    funcpars[2] = emispar;
    funcpars[3] = inputplleindex;
    funcpars[4] = inputbplhiindex;
    funcpars[5] = inputbpln;
    funcpars[6] = inputbplEbreak;
    funcpars[7] = inputlogEcut;*/
    ...
    TF1 qe_mod_diff("qe_mod_diff",QE_mod,(log_prot_gmin-0.5),(log_prot_gmax+0.5),8);
...
//int main(int argc, char **argv){
  ...
  std::string prefix = "normalized_pf_"; string suffix = ".txt"; 
  int i, j, k;
  ...
  j = 0;  
    ...
    // To run the current model, type: |> ./<name of executable>.out 1.0 2.0 2.1 0.0 0.0 0.0 15.0
...
//  double emispar = atof(argv[1]), specpar = atof(argv[2]), inputplleindex = atof(argv[3]), inputbplhiindex = atof(argv[4]), inputbpln = atof(argv[5]), inputbplEbreak = pow(10.0,atof(argv[6])), inputlogEcut = pow(10.0,atof(argv[7]));
  ...
  double inputbplhiindex = 0.0;
  double inputbpln = 0.0;
  double inputbplEbreak = 0.0;
  double exponent;
  ...
  double logNnotprime, t, b, y;
    ...
    double logbinsize = .1;
    double loggamma_start = 0.1;
    double loggamma = log_prot_gmin + logbinsize;
    double ne_ss, normalization, qekininteg;
    double ne_sspars[8];
    ...
    switch (emisflag) {
        case 1: // NGC 253 -- Lacki et al. 2012 hadronic
            regBfield = 4.e-4; // [Gauss]
            irscaleheight = 5.0*pc; //[cm]
            sneff_proton = 0.1;
            //ofstream fne("Tests/Lacki/400muG/ne_prot_ss_finite_exp_cutoff_w_args.dat");
            //sprintf(protdistfilename,"/Users/CEE/Desktop/NGC_253/ne_prot_ss_finite_exp_cutoff_w_args.dat");
            //TFile protdistrootfile("Tests/Lacki/400muG/protdistmaster.root","recreate");
            sprintf(rootfilename,"/Users/CEE/Desktop/NGC_253/protdistmaster.root");
            break;
        case 2: // NGC 253 -- Lacki et al. 2012 leptonic
            regBfield = 5.e-5; // [Gauss]
            irscaleheight = 50.0*pc;
            sneff_proton = 0.2;
            //ofstream fne("Tests/Lacki/50muG/ne_prot_ss_finite_exp_cutoff_w_args.dat");
            //sprintf(protdistfilename,"/Users/CEE/Desktop/NGC_253/ne_prot_ss_finite_exp_cutoff_w_args.dat");
            //TFile protdistrootfile("Tests/Lacki/50muG/protdistmaster.root","recreate");
            sprintf(rootfilename,"/Users/CEE/Desktop/NGC_253/protdistmaster.root");
            break;
    }
    
    //ofstream fne(protdistfilename);
    ofstream fne;
    TFile protdistrootfile(rootfilename,"update");
    ...
    //double galBfield = (atof(argv[1])*(1.e-6)), emispar = atof(argv[2]), specpar = atof(argv[3]), inputplleindex = atof(argv[4]), inputbplhiindex = atof(argv[5]), inputbpln = atof(argv[6]), inputbplEbreak = pow(10.0,atof(argv[7])), inputlogEcut = pow(10.0,atof(argv[8]));
    
    // Find normalization for proton injection spectrum
    
    sprintf(graphname,"graph_log_prot_gmax_%.1f_eff_%.1f",log_prot_gmax,sneff_proton);
    sprintf(funcname,"func_log_prot_gmax_%.1f_eff_%.1f",log_prot_gmax,sneff_proton);
    ...
    //cout << regBfield << " " << irscaleheight << " " << sneff_proton << endl;
    
    normalization = sneff_proton*snenergy*snrate*1./(volirregion*qekininteg); // dimensionless
    
    //cout << normalization << " " << qekininteg << endl;
    
    //ne_sspars[0] = normalization;
    //ne_sspars[1] = regBfield;

    TDirectory *rootfiledir;
    
   for (i=0, inputplleindex=1.9; inputplleindex<=2.05; inputplleindex+=0.025, ++i)
  {  
    ...
    switch (specflag) {
        case 1: // Power Law
            sprintf(rootdirname,"ne_prot_ss_finite_B_%.1f_plindex_%.1f",(regBfield*1.e6),inputplleindex);
            sprintf(protdistfilename,"/Users/CEE/Desktop/NGC_253/ne_prot_ss_finite_B_%.1f_plindex_%.1f.dat",(regBfield*1.e6),inputplleindex);
            break;
        case 2: // Cutoff Power Law
            sprintf(rootdirname,"ne_prot_ss_finite_B_%.1f_plindex_%.3f_Ecut_%.1f",(regBfield*1.e6),inputplleindex,log10(inputlogEcut));
            ...
            break;
        case 3: // Smoothly Broken Power Law
            sprintf(rootdirname,"ne_prot_ss_finite_B_%.1f_leindex_%.1f_hiindex_%.1f_n_%.1f_Eb_%.1f",(regBfield*1.e6),inputplleindex,inputbplhiindex,inputbpln,log10(inputbplEbreak));
            sprintf(protdistfilename,"/Users/CEE/Desktop/NGC_253/ne_prot_ss_finite_B_%.1f_leindex_%.1f_hiindex_%.1f_n_%.1f_Eb_%.1f.dat",(regBfield*1.e6),inputplleindex,inputbplhiindex,inputbpln,log10(inputbplEbreak));
            break;
        case 4: // Cutoff Smoothly Broken Power Law
            sprintf(rootdirname,"ne_prot_ss_finite_B_%.1f_leindex_%.1f_hiindex_%.1f_n_%.1f_Eb_%.1f_Ecut_%.1f",(regBfield*1.e6),inputplleindex,inputbplhiindex,inputbpln,log10(inputbplEbreak),log10(inputlogEcut));
            sprintf(protdistfilename,"/Users/CEE/Desktop/NGC_253/ne_prot_ss_finite_B_%.1f_leindex_%.1f_hiindex_%.1f_n_%.1f_Eb_%.1f_Ecut_%.1f.dat",(regBfield*1.e6),inputplleindex,inputbplhiindex,inputbpln,log10(inputbplEbreak),log10(inputlogEcut));
            break;
        default:
            sprintf(rootdirname,"ne_prot_ss_finite_B_%.1f_plindex_%.1f",(regBfield*1.e6),inputplleindex);
            sprintf(protdistfilename,"/Users/CEE/Desktop/NGC_253/ne_prot_ss_finite_B_%.1f_plindex_%.1f.dat",(regBfield*1.e6),inputplleindex);
        ...
        fne.open(protdistfilename);
        
        protdistrootfile.cd();
        rootfiledir = (TFile*)protdistrootfile.mkdir(rootdirname);
        rootfiledir->cd();
    
        TGraph protdistgraph;
        TF1 protdistfunc(funcname,NE_finite_time,loggamma_start,(log10(inputlogEcut*1./protonmass)+1.0),8);
        protdistfunc.SetParameters(ne_sspars);
    
        std::string file = prefix + std::to_string (i) + std::to_string (j) + std::to_string (k) + suffix;
        ...
        myfile << "# " << "Output file of: 'normalized_proton_code' " << "\t" << "Round " << i << j << k << endl;
        myfile << "# " << "for normalization = " << normalization << "," << " specpar = " << specpar << "," << " emispar = "<< emispar << "," << " inputplleindex = " << inputplleindex << "," << " and inputlogEcut = " << inputlogEcut << endl;
//        myfile << "Given : " << "[" << argv[1] << "  " << argv[2] << "  " << argv[3] << "  " << argv[4] << "  " << argv[5] << "  " << argv[6] << "  " << argv[7] << "]" <<endl;
        myfile << '#' << '\n';
        myfile << "# " << "loggamma" << "\t" << "t" <<"\t" << "\t" << "\t" << "y" << "\t" <<"\t" << "b" << "\t" << "\t" << "inputplleindex"<< endl;;
    
    
        logNnotprime = log10(NE_finite_time (&loggamma_start, ne_sspars));
        
        num = 0;
        for (loggamma=.1; loggamma<=log_prot_gmax; loggamma+=logbinsize)
            {
                ne_ss= NE_finite_time(& loggamma, ne_sspars);
                
                protdistgraph.SetPoint(num,loggamma,ne_ss);
                ...
                if (loggamma <= 3.0) myfile << loggamma << "\t" << "\t" << t << "\t" << "\t" << y << "\t" << "\t" << b << "\t" << "\t" << inputplleindex << endl; //to save to a file, use myfile instead of cout   
        ...
        protdistgraph.SetName(graphname);
        protdistgraph.Write();
        protdistfunc.Write();
    }
